...
(
    ...
    if [ "$jenkins" == true ] ;
    then
        terraform -chdir=./components/ apply -target=module.jenkins --auto-approve ;
    fi

        if [ "$nexus" == true ] ;
    then
        terraform -chdir=./components/ apply -target=module.nexus --auto-approve ;
    fi
    ...
    if [ "$tekton" == true ] ;
    then
        terraform -chdir=./components/ apply -target=module.tekton --auto-approve ;
    fi
